						...
						if(params.length)
						{
							$.each(params, function(i, val) 	// parse param to right datatype
							{
								if(val.length)
								{
									...
									console.log('WALKING THRU PARAMS', val, !!val.match(/^"/));

									if(val.match(/^"/))		// escaped with double quotes
									{
										var k = i + 0;

										while( !params[k].match(/"$/) )
										{
											k++;
											console.log('k', k, 'i', i, 'pk', params[k]);
											val += params.splice(k, 1);
										}
